 ...
 * - px-2 (8px) horizontal padding on mobile
 * - No padding on screens with sufficient width (content + 15px margin on each side)
...
export default function Container({ children, className = "", maxWidth = "540px" }: ContainerProps) {
  ...
  const breakpointClass = maxWidth === "868px" ? "min-[898px]:px-0" : "min-[560px]:px-0";
  
  return (
    <div className={`mx-auto w-full ${maxWidthClass} px-2 ${breakpointClass} ${className}`}>
      {children}
    </div>
  );
}
